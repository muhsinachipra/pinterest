...
const PinPage = ({ user }) => {
  ...
  const editHandler = () => {
    console.log('pin in edithadler ', pin)
    setTitle(pin.title);
    setPinValue(pin.pin);
    setEdit(!edit);
  };
  ...
  useEffect(() => {
    if (pin) {
      setLikeCount(pin.likes.length); // No need for `|| 1`, it should reflect the actual count.
  ...
  useEffect(() => {
    console.log('pin: ', pin);
    if (pin && user) {
      setIsLiked(pin.likes && pin.likes.includes(user._id));
    }
  }, [pin, user]);
  ...
  const likeHandler = async () => {
    if (isLiked) {
      ...
      setLikeCount((prev) => Math.max(prev - 1, 0)); // Ensure like count does not go negative
    } else {
